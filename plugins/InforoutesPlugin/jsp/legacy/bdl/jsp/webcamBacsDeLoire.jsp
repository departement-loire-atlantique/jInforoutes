...
<div class="webcam-bac-de-loire page">
   <h2><%= box.getDisplayTitle() %></h2>
	...
	<div class="image-webcam"><img id="img1" src="<%=propWebCam1[1] %>" alt="image des bacs de <%=propWebCam1[0] %>" title="image des bacs de <%=propWebCam1[0] %>"/></div>
	...
	<div class="image-webcam"><img id="img2" src="<%=propWebCam2[1] %>"  alt="image des bacs de <%=propWebCam2[0] %>" title="image des bacs de <%=propWebCam2[0] %>"/></div>
    ...
    function refreshLastImageWebCam1(){
		...
		now=new Date();
		if(webCamActive1){
			var img = jQuery("<img />").attr("src", "<%=propWebCam1[1] %>?timestamp=" + now.getTime())
				.load(function() {
    ...
    function refreshLastImageWebCam2(){
    	if(webCamActive2){
    	now=new Date();
   		var img = jQuery("<img />").attr("src", "<%=propWebCam2[1] %>?timestamp=" + now.getTime())
		    .load(function() {
		        if (!this.complete || typeof this.naturalWidth == "undefined" || this.naturalWidth == 0) {
